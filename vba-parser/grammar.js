...
const PREC = {
  ...
  exp: 14,         // ^ (exponentiation) - highest arithmetic
  mul: 13,         // *, /
  intdiv: 12,      // \ (integer division)
  mod: 11,         // Mod
  add: 10,         // +, -
  not: 9,          // Not (logical NOT) - above concat in VBA
  concat: 8,       // &
  relational: 7,   // >, <, >=, <=
  equality: 6,     // =, <>
  and: 5,          // And
...
module.exports = grammar({
  ...
  conflicts: $ => [
    ...
    [$.with_statement],  // Added for With statement optional trailing newline
  ],
  ...
  rules: {
    ...
    // Parameter list: parentheses with comma-separated parameters
    parameter_list: $ => seq(
      '(',
    ...
    // Individual parameter with modifiers
    parameter: $ => seq(
      optional(field('modifier', choice(
        token(/Optional/i),
        token(/ParamArray/i)
      ))),
      optional(field('passing', choice(
        token(/ByRef/i),
        token(/ByVal/i)
      ))),
      field('name', $.identifier),
    ...
    // Function Definition: Function Name(params) As Type ... End Function
    function_definition: $ => seq(
      optional(field('visibility', choice(token(/Public/i), token(/Private/i)))),
      token(/Function/i),
      field('name', $.identifier),
      optional(field('params', $.parameter_list)),
      optional(seq(token(/As/i), field('return_type', choice($.primitive_type, $.identifier)))),
      /\r?\n/,
      repeat($.statement),
      token(/End\s+Function/i),
      /\r?\n/
    ),

    // Property Get: Property Get Name(params) As Type ... End Property
    property_get: $ => seq(
      optional(field('visibility', choice(token(/Public/i), token(/Private/i)))),
      token(/Property/i),
      token(/Get/i),
      ...
      /\r?\n/,
      repeat($.statement),
      token(/End\s+Property/i),
      /\r?\n/
    ),

    // Property Let: Property Let Name(params) ... End Property
    property_let: $ => seq(
      optional(field('visibility', choice(token(/Public/i), token(/Private/i)))),
      token(/Property/i),
      token(/Let/i),
    ...
    // Property Set: Property Set Name(params) ... End Property
    property_set: $ => seq(
      optional(field('visibility', choice(token(/Public/i), token(/Private/i)))),
      token(/Property/i),
      token(/Set/i),
    ...
    // ReDim statement: ReDim [Preserve] var(bounds) [As Type]
    redim_statement: $ => seq(
      token(/ReDim/i),
      optional(field('preserve', token(/Preserve/i))),
      commaSep($.redim_variable),
      /\r?\n/
    ),
    ...
    redim_variable: $ => seq(
      field('name', $.identifier),
      '(',
      field('bounds', commaSep($.redim_bound)),
      ')',
      optional(seq(token(/As/i), field('type', choice($.primitive_type, $.identifier))))
    ),

    redim_bound: $ => choice(
      // Range: lower To upper
      seq(field('lower', $.expression), token(/To/i), field('upper', $.expression)),
      // Single bound: 0 To upper (implicit lower = 0)
    ...
    lvalue: $ => choice(
      ...
      $.with_member_access,            // With member: .prop (within With blocks)
      $.with_method_call,              // With method: .Method(args) (within With blocks)
   ...
   // Call statement: Call Func(args)
    ...
    expression: $ => choice(
      ...
      $.function_call,
      $.property_access,
      $.with_member_access,    // Added: .Property (within With blocks)
      $.with_method_call,      // Added: .Method(args) (within With blocks)
    ...
    property_access: $ => prec.left(PREC.member, seq(
      field('object', choice(
        ...
        $.with_method_call,      // Added: .Method(args).Property
        $.with_member_access     // Added: .Property.SubProperty
      )),
      '.',
      field('property', $.identifier)
    )),

    // With member access: .Property (within a With block)
    with_member_access: $ => prec.left(PREC.member, seq(
      '.',
      field('property', $.identifier)
    )),

    // With method call: .Method(args) (within a With block)
    with_method_call: $ => prec.left(PREC.call, seq(
      '.',
    ...
    binary_expression: $ => choice(
      ...
      prec.right(PREC.exp, seq($.expression, token('^'), $.expression)),
      // Multiplication and division
      prec.left(PREC.mul, seq($.expression, token(choice('*', '/')), $.expression)),
      // Integer division
      prec.left(PREC.intdiv, seq($.expression, token('\\'), $.expression)),
      // Modulus
      prec.left(PREC.mod, seq($.expression, $.keyword_Mod, $.expression)),
      ...
      prec.left(PREC.xor, seq($.expression, $.keyword_Xor, $.expression)),
      prec.left(PREC.eqv, seq($.expression, $.keyword_Eqv, $.expression)),
      prec.left(PREC.eqv, seq($.expression, $.keyword_Imp, $.expression))
    ),
    unary_expression: $ => choice(
      prec.right(PREC.exp + 1, seq(field('operator', choice('-', '+')), field('argument', $.expression))),
      prec.right(PREC.not, seq(field('operator', $.keyword_Not), field('argument', $.expression)))
    ),
    ...
    keyword_Xor:     $ => token(/Xor/i),
    keyword_Eqv:     $ => token(/Eqv/i),
    keyword_Imp:     $ => token(/Imp/i),
